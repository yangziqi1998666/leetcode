...
class Solution {
    public int compareVersion(String version1, String version2) {
        // split two string into two array
        ...
        // the sign of exiting loop is i out of the range of max(v1_length,v2_length)
        while(i < v1_length || i < v2_length){
            // initialize the current position
            int x = i < v1_length ? Integer.valueOf(v1[i]) : 0;
            int y = i < v2_length ? Integer.valueOf(v2[i]) : 0;
            // loop element
            i++;
            // euqal
            if(x == y)
        ...
        // they are equal until the last position
        return 0;
    }
}
